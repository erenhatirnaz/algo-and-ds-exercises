 ...
 * 4 arguments. The arguments consist of an integer array and 3 integer values.
 * The array has to be a sorted array. Because, the algorithm only works with
 * sorted array. The other 3 arguments are: left, right and key. left and right
 * is values of minimum(left) and maximum(right) for the binary search
 * algorithm. The key is value of sought. The algorithm finds the middle of the
 * left and the right values at each step and continues until the result is
 * equal to the key. This function returns the index value of the key. If the
 * key doesn't exist in the array, it returns -1. This function runs recursive.
 */
